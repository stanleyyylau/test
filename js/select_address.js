...
* @Last Modified time: 2016-11-02 16:33:05
*/
// 必须存在默认值
/*
...
var address = {
  ...
  init: function (argument) {
    ...
    $('.J_scrollerConfirm').on('click', function(event) {
  ...
  getStreet: function(id){
    ...
    if (id && !that.isLoading) {
      that.isLoading = true
      streetTip = common.tips('加载中...', '300000')
      $.ajax({
        url: '/path/to/file',
        type: 'POST',
        dataType: 'json',
        ...
        success: function(ret){
          if (ret.status) {
            streetTip.dialog.close()
            ret.data.length > 0 ? that.streetData = read(ret.data) : that.streetData = {name: '暂不选择', value: '000000'}

            that.scrollerStreet(cityDefauleVal.street.code)
            address.show('J_scroller2')
          } else {
            streetTip.dialog.update({
              autoClose: 1500,
  ...
  confirm: function (dom, callback) {
    ...
    $('#J_selectCityTxt').html(txt).removeClass('placeholder');
  ...
  streetConfirm: function (){
    var ret = $('.J_scroller2').find('.scroller-item-selected')
